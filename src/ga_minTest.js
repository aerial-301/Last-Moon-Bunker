export var GA = {
    create(setup) {
        ...
        function capturePreviousSpritePositions() {
            g.stage.children.forEach(function (s) {
                setPosition(s);
            });
            function setPosition(s) {
                ...
                if (s.children && s.children.length > 0)
                    s.children.forEach(child => setPosition(child));
            }
        }
        function update() {
            if (g.state && !g.paused) {
        ...
        g.render = function (canvas, lagOffset) {
            ...
            function displaySprite(s) {
                if (s.visible && s.gx < canvas.width + s.width && s.gx + s.width >= -s.width && s.gy < canvas.height + s.height && s.gy + s.height >= -s.height) {
                    ctx.save();
                    if (g.interpolate) {
                        if (s._previousX !== undefined) {
                            s.renderX = (s.x - s._previousX) * lagOffset + s._previousX;
                        }
                        else {
                            s.renderX = s.x;
                        }
                        if (s._previousY !== undefined) {
                            s.renderY = (s.y - s._previousY) * lagOffset + s._previousY;
                        }
                        else {
                            s.renderY = s.y;
                        }
