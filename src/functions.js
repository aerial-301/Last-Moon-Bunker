    ...
    collider.obstacles.forEach(obst => {
        if (g.hitTestRectangle(collider, obst)) {
            ...
            if (obst == collider.collidedWith) {
                ...
                if (H)
                    collider.isCollidingH = false;
                else
                    collider.isCollidingV = false;
...
const globalAngle = (a, b) => {
    return Math.atan2((b.gx + b.halfWidth - a.gx + a.halfWidth), (b.gy + b.halfHeight - a.gy + a.halfHeight));
};
const tempAngle = (a, b, bOffsetX = 0, bOffsetY = 0) => {
    return Math.atan2((b.centerX + bOffsetX - a.centerX), (b.centerY + bOffsetY - a.centerY));
};
